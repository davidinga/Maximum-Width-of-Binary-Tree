...
func widthOfBinaryTree(_ root: TreeNode?) -> Int {
    ...
    var maxWidth = 0
    var queue: Deque<(index: Int, node: TreeNode)> = []
    queue.append((0, root))

    while queue.count > 0 {
        let qCount = queue.count
        let (start, end) = (queue.first!.index, queue.last!.index)
        maxWidth = max(end - start + 1, maxWidth)

        for _ in 0..<qCount {
            var (i, node) = queue.removeFirst()
            i -= start // overflow fix

            if let left = node.left {
                queue.append((i * 2, left))
            }

            if let right = node.right {
                queue.append((i * 2 + 1, right))
            }
        }
    }
